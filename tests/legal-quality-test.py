...
RULES = [
    {
        "id": "NO_TUICION",
        "check": lambda text: (
            "tuición" in text.lower()
            and "cuidado personal" not in text.lower()
        ),
        "severity": "fail",
    ...
    },
    {
        ...
        "check": lambda text: bool(
            re.search(r'\[.+?\]', text) and "[DONE]" not in text
        ),
        "severity": "fail",
    ...
    {
        "id": "NO_DEM",
        "trigger_query": "migracion_ser mig",
        "check": lambda text: bool(re.search(r'\bdem\b', text.lower())),
        "severity": "fail",
